...
public class JDBC_Pool {
    ...
    public void init() {
        ...
        pool.setUrl("jdbc:mysql://" + const_value.SqlAdd + ":3306/db_klzy_cloud?characterEncoding=utf8&autoReconnect=true"); // 设置url
    ...
    private String parseResultSet(ResultSet rs, ResultSetMetaData rsmd, int type, int i) throws SQLException {
        ...
        switch (type) {
            ...
            case Types.TIMESTAMP:
                Timestamp tam = rs.getTimestamp(i + 1);
                if (tam  != null) {
                    DateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    resStr = sdf.format(tam);
                }else{
                    resStr = null;
                }
                break;
    ...
    public int execute(String sql, List<Object> param) {
        ...
        try {
            ...
            e.printStackTrace();
            if (e.toString().indexOf("PRIMARY")>0 || e.toString().indexOf("UNIQUE")>0 )
                return -13;
            return -1;
        } finally {
            // 关闭连接 释放资源
     ...
     * @return
     */
    public int execute(Connection conn, String sql, List<Object> param,int i) {
        // 处理结果
        int result = 0;
        PreparedStatement ps = null;
        ResultSet rs = null;
     ...
     * @return
     */
    public List<Map<String, Object>> queryForSpaMap(String sql, List<Object> param) {
        List<Map<String, Object>> rul_map = new ArrayList<>();
        Connection conn = null;
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ...
            while (rs.next()) {
                ...
                for (int j = 0; j < cols; j++) {
                    String colName = rsmd.getColumnLabel(j + 1);
                    if (j == 3) {
                        java.sql.Blob blob = rs.getBlob("value");
                        long l = blob.length();
                        ...
                        bin.read(b);
                        map.put(colName.toLowerCase(), b);
                    } else {
                        int type = rsmd.getColumnType(j + 1);
                        val = parseResultSet(rs, rsmd, type, j);
                        map.put(colName.toLowerCase(), val);
                    }
                }
                rul_map.add(map);
